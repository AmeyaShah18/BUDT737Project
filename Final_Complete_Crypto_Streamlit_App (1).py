...
if 'crypto_df' in st.session_state:
    ...
    st.subheader("Time Series Trend Proxy: Price vs Rank")
    trend = alt.Chart(df).mark_line().encode(
        x="cmc_rank", y="quote_USD_price", color="name"
    ).properties(title="Price by Coin Rank")
